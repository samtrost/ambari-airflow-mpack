...
class AirflowScheduler(Script):
	...
	def install(self, env):
		...
		Execute(format("/opt/anaconda/bin/pip install --upgrade {airflow_pip_params} pip"))
		Execute(format("/opt/anaconda/bin/pip install --upgrade {airflow_pip_params} setuptools"))
		Execute(format("/opt/anaconda/bin/pip install --upgrade {airflow_pip_params} docutils pytest-runner Cython==0.28"))
		Execute(format("export SLUGIFY_USES_TEXT_UNIDECODE=yes && /opt/anaconda/bin/pip install --upgrade {airflow_pip_params} --ignore-installed apache-airflow[all]==1.10.0"))
		Execute(format("export SLUGIFY_USES_TEXT_UNIDECODE=yes && /opt/anaconda/bin/pip install --upgrade {airflow_pip_params} --ignore-installed apache-airflow[celery]==1.10.0"))
